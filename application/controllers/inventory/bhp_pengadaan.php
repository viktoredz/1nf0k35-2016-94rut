...
class Bhp_pengadaan extends CI_Controller {
	...
	function edit($id_pengadaan=0){
		...
		if($this->form_validation->run()== FALSE){
			...
			$data['kodepuskesmas'] = $this->puskesmas_model->get_data();
			$data['kodestatus'] = $this->bhp_pengadaan_model->get_data_status();
			$data['kodestatus_inv'] = $this->bhp_pengadaan_model->pilih_data_status('status_inventaris');
	...
	function dodelpermohonan($pengadaan=0,$kode=0,$kembarproc=0){
		if($this->bhp_pengadaan_model->delete_entryitem($kode,$kembarproc)){
				
		}else{
			$this->session->set_flashdata('alert', 'Delete data error');
		}
				$dataupdate['jumlah_unit']= $this->bhp_pengadaan_model->sum_unit($kode)->num_rows();
				$dataupdate['nilai_pengadaan']= $this->bhp_pengadaan_model->sum_jumlah_item( $kode,'harga');
	...
	public function edit_barang($id_pengadaan=0,$kd_inventaris=0,$kode_proc=0)
	{
       ...
       // $this->form_validation->set_rules('keterangan_pengadaan', 'Keterangan', 'trim|required');
		...
		if($this->form_validation->run()== FALSE){
			...
			$kodebarang_ = substr($kd_inventaris, -14,-12);
	   		if($kodebarang_=='01') {
	   			$data = $this->bhp_pengadaan_model->get_data_barang_edit_table($kd_inventaris,'inv_inventaris_barang_a'); 
	   			$data['pilihan_satuan_barang_']			= $this->bhp_pengadaan_model->get_data_pilihan('satuan');
	   			$data['pilihan_status_hak_']			= $this->bhp_pengadaan_model->get_data_pilihan('status_hak');
	   			$data['pilihan_penggunaan_']			= $this->bhp_pengadaan_model->get_data_pilihan('penggunaan');
	   		}else if($kodebarang_=='02') {
	   			$data = $this->bhp_pengadaan_model->get_data_barang_edit_table($kd_inventaris,'inv_inventaris_barang_b'); 
	   			$data['pilihan_bahan_']				= $this->bhp_pengadaan_model->get_data_pilihan('bahan');
	   			$data['pilihan_satuan_']				= $this->bhp_pengadaan_model->get_data_pilihan('satuan');
	   		}else if($kodebarang_=='03') {
	   			$data = $this->bhp_pengadaan_model->get_data_barang_edit_table($kd_inventaris,'inv_inventaris_barang_c'); 
	   			$data['pillihan_status_hak_']		= $this->bhp_pengadaan_model->get_data_pilihan('status_hak');
	   			$data['pilihan_kons_tingkat_']		= $this->bhp_pengadaan_model->get_data_pilihan('kons_tingkat');
	   			$data['pilihan_kons_beton_']			= $this->bhp_pengadaan_model->get_data_pilihan('kons_beton');
	   		}else if($kodebarang_=='04') {
	   			$data = $this->bhp_pengadaan_model->get_data_barang_edit_table($kd_inventaris,'inv_inventaris_barang_d'); 
	   			$data['pilihan_status_tanah_']		= $this->bhp_pengadaan_model->get_data_pilihan('status_hak');
	   		}else if($kodebarang_=='05') {
	   			$data = $this->bhp_pengadaan_model->get_data_barang_edit_table($kd_inventaris,'inv_inventaris_barang_e'); 
	   			$data['pilihan_budaya_bahan_']		= $this->bhp_pengadaan_model->get_data_pilihan('bahan');
	   			$data['pilihan_satuan_']				= $this->bhp_pengadaan_model->get_data_pilihan('satuan');
   			}else if($kodebarang_=='06') {
   				$data = $this->bhp_pengadaan_model->get_data_barang_edit_table($kd_inventaris,'inv_inventaris_barang_f'); 
   				$data['pilihan_konstruksi_bertingkat_']= $this->bhp_pengadaan_model->get_data_pilihan('kons_tingkat');
	   			$data['pilihan_konstruksi_beton_']	= $this->bhp_pengadaan_model->get_data_pilihan('kons_beton');
	   			$data['pilihan_status_tanah_']		= $this->bhp_pengadaan_model->get_data_pilihan('status_hak');
   			}
   			//$data = $this->bhp_pengadaan_model->get_data_barang_edit($id_barang,$kd_proc,$kd_inventaris); 
   			$data['kodebarang']		= $this->bhp_pengadaan_model->get_databarang();
   			$data['kodestatus_inv'] = $this->bhp_pengadaan_model->pilih_data_status('status_inventaris');
			$data['action']			= "edit";
			$data['kode']			= $kd_inventaris;
			...
			$tanggal = $this->bhp_pengadaan_model->tanggal($id_pengadaan);
			$data_update = array(
					'nama_barang' 			=> $this->input->post('nama_barang'),
					'harga' 				=> $this->input->post('harga'),
			...
			//for($i=1;$i<=$jumlah;$i++){
			//	$id = $this->bhp_pengadaan_model->insert_data_from($id_barang,$kode_proc,$tanggal_diterima,$id_pengadaan);
        		...
        		$kodebarang_ = substr($keyinv->id_mst_inv_barang,0,2);
		   		if($kodebarang_=='01') {	
				 	...
				 	$this->db->delete('inv_inventaris_barang_a');
		   			/*$jumlah = $this->bhp_pengadaan_model->jumlahtable('inv_inventaris_barang_a',$keyinv->id_inventaris);
		   			if ($jumlah>0) {
					...
					//}
				 	...
				 	$this->db->delete('inv_inventaris_barang_b');
		   			/*$jumlah = $this->bhp_pengadaan_model->jumlahtable('inv_inventaris_barang_b',$keyinv->id_inventaris);
		   			if($jumlah>0){
					...
					//}
				 	...
				 	$this->db->delete('inv_inventaris_barang_c');
		   			/*$jumlah = $this->bhp_pengadaan_model->jumlahtable('inv_inventaris_barang_c',$keyinv->id_inventaris);
		   			if ($jumlah>0) {
					...
					//}
				 	...
				 	$this->db->delete('inv_inventaris_barang_d');
		   			/*$jumlah = $this->bhp_pengadaan_model->jumlahtable('inv_inventaris_barang_d',$keyinv->id_inventaris);
		   			if($jumlah>0){
		   				$tanggal = explode("/",$this->input->post('dokumen_tanggal'));
			   			$dokumen_tanggal = $tanggal[2].'-'.$tanggal[1].'-'.$tanggal[0];
			   			$values = array(
					...
					//}
				 	...
				 	$this->db->delete('inv_inventaris_barang_e');
		   			/*$jumlah = $this->bhp_pengadaan_model->jumlahtable('inv_inventaris_barang_e',$keyinv->id_inventaris);
		   			if ($jumlah>0) {
					...
					/*$jumlah = $this->bhp_pengadaan_model->jumlahtable('inv_inventaris_barang_f',$keyinv->id_inventaris);
					if($jumlah>0){
			...
			if($simpan==true){
				...
				$dataupdate__['nilai_pengadaan']= $this->bhp_pengadaan_model->sum_jumlah_item( $id_pengadaan,'harga');
				$dataupdate__['jumlah_unit']= $this->bhp_pengadaan_model->sum_unit($id_pengadaan)->num_rows();
