...
class Bhp_retur extends CI_Controller {
	...
	function daftar_bhp(){
		...
		die($this->parser->parse("inventory/bhp_retur/show_bhp",$data,true));
	}
	...
	{	
		...
		if($this->form_validation->run()== FALSE){
			...
			die($this->parser->parse('inventory/bhp_retur/barang_form', $data));
		}else{
	...
	{	
		...
		if($this->form_validation->run()== FALSE){
			...
			die($this->parser->parse('inventory/bhp_retur/barang_form_master', $data));
		}else{
	...
	{	
		...
		if($this->form_validation->run()== FALSE){
			...
			$this->db->where('code','P'.$kodepuskesmas);
			$data['kodepuskesmas'] = $this->puskesmas_model->get_data();
			die($this->parser->parse('inventory/bhp_retur/detail_form', $data));
		}else{
			if($simpan=$this->bhp_opname_model->insertdata()){
				$id=$this->input->post('id_mst_inv_barang');
				die("OK|$id|Tersimpan");
  	...
  	public function timeline_pengeluaran_barang($id_barang = 0){
  		...
  		echo $this->parser->parse("inventory/bhp_retur/kondisi",$data);
